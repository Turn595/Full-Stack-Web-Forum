...
switch ($_SERVER['REQUEST_METHOD']){
    case 'GET':
        $resultToEncode = ArticleController::getArticles($entityManager, $requestData);
        break;
    case 'POST'://Send data to the DB, can create the same resource multiple times.
        $resultToEncode = ArticleController::postArticles($entityManager, $requestData, new Article());
        break;
    case 'PUT'://Send data to the DB, does not create the same resource multiple times.
        $resultToEncode = ArticleController::putArticles($entityManager, $requestData,
            //the entity manager attempts to find a student with the id, null if it doesnt exist.
            $entityManager->find(Article::class, $requestData['id']));
        break;
    case 'DELETE':
        $resultToEncode = ArticleController::deleteArticles($entityManager, $requestData,
            $entityManager->find(Article::class, $requestData['id']));
        break;
